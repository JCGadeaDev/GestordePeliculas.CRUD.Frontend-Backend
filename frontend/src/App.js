...
function MovieApp() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setError('No se pudo conectar al backend. Asegurate de que esté corriendo.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleAdd    = async (d)      => { await movieService.create(d);        await load(); };
  const handleUpdate = async (id, d)  => { await movieService.update(id, d);   await load(); if (detailMovie?._id === id) setDetailMovie(null); };
  const handleDelete = async (id)     => { await movieService.remove(id);       await load(); };

  const stats    = useMemo(() => computeStats(movies), [movies]);
  const genres   = useMemo(() => extractGenres(movies), [movies]);
  const filtered = useMemo(() => applyFilters(movies, { search, status, genre, sortBy }), [movies, search, status, genre, sortBy]);
    ...
    <div className="app">
      ...
      <main className="app-main">
        ...
        <MovieGrid
          movies={filtered}
          loading={loading}
          onEdit={isAdmin ? setEditingMovie : null}
      ...
      {editingMovie && isAdmin && (
        <EditMovieModal movie={editingMovie} onSave={handleUpdate} onClose={() => setEditingMovie(null)} />
      )}
      {detailMovie && (
        <MovieDetailModal
          movie={detailMovie}
...
function AppLoader() {
  ...
  return user ? <MovieApp /> : <LoginPage />;
}
